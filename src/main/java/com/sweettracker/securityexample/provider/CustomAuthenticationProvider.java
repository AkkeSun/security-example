...
public class CustomAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) {
        ...
        if (!StringUtils.hasText(secretKey)) {
            ...
            userDetails = memberService.loadUserByUsernameAndSecretKey(username);
            if (!passwordEncoder.matches(secretKey, userDetails.getPassword())) {
                throw new BadCredentialsException("Invalid Username or secretKey");
            }
        }
    ...
    @Override
    // Authentication과 해당 토큰이 같을 때 구동 (필터 커스텀시 변경해줍니다)
